...
const i18n = {
  en: {
    ...
    profilePoint3Subtitle: 'Track consumption, rewards, and arcade scores with charming Astro charts.',
...
function translate(key){
  ...
  const dict = i18n[currentLang] || fallbackDict;
...
function applyTranslations(pageType){
  ...
  if (pageType === 'tasks') {
    setText('placeholderBadge', 'badgeAlpha');
    setText('pageTitle', 'tasksTitle');
    setText('pageSubtitle', 'tasksSubtitle');
    setText('pointOneTitle', 'tasksPoint1Title');
    setText('pointOneSubtitle', 'tasksPoint1Subtitle');
    setText('pointTwoTitle', 'tasksPoint2Title');
    setText('pointTwoSubtitle', 'tasksPoint2Subtitle');
    setText('pointThreeTitle', 'tasksPoint3Title');
    setText('pointThreeSubtitle', 'tasksPoint3Subtitle');
    setText('ctaLabel', 'tasksCta');
  } else if (pageType === 'profile') {
    setText('placeholderBadge', 'profileBadge');
    setText('pageTitle', 'profileTitle');
    setText('pageSubtitle', 'profileSubtitle');
    setText('pointOneTitle', 'profilePoint1Title');
    setText('pointOneSubtitle', 'profilePoint1Subtitle');
    setText('pointTwoTitle', 'profilePoint2Title');
    setText('pointTwoSubtitle', 'profilePoint2Subtitle');
    setText('pointThreeTitle', 'profilePoint3Title');
    setText('pointThreeSubtitle', 'profilePoint3Subtitle');
    setText('ctaLabel', 'profileCta');
  }
}
...
function setLanguage(lang, pageType){
  ...
  if (btn) {
    ...
    btn.textContent = isFa ? translate('languageFA') : translate('languageEN');
...
function initLanguage(pageType){
  let saved = null;
  try{ saved = localStorage.getItem('lang') || null; }catch(_){}
  let guess = 'en';
  try{
  ...
  const btn = document.getElementById('langSwitch');
  if (btn) {
    btn.onclick = () => setLanguage(currentLang === 'en' ? 'fa' : 'en', pageType);
  }
}
...
function initBottomNav(currentPage){
  ...
  function updateBubblePosition(){
    ...
    const containerEl = active.parentElement;
  ...
  function startNavigation(url, item, useTelegram){
    ...
    if (prefersReducedMotion()) {
      try{ sessionStorage.removeItem(navTransitionStorageKey); }catch(_){}
      if (useTelegram && tg && tg.openLink) tg.openLink(url); else window.location.href = url;
      return;
    }
    const origin = computeNavOriginFromRect(item.getBoundingClientRect());
    applyNavTransitionOrigin(origin);
    storeNavTransitionOrigin(origin);
  ...
  navItems.forEach(item => {
    item.addEventListener('click', () => {
      ...
      if (page === activePage) {
        ...
        setTimeout(() => bubble.classList.remove('moving'), 360);
        return;
      }
      ...
      if (url) {
        const useTelegram = page === 'arcade';
        startNavigation(url, item, useTelegram);
      }
    });
  });
  ...
  return {
    triggerNavigation(url){
      const target = navItems.find(item => item.getAttribute('data-url') === url);
      if (target) target.click(); else window.location.href = url;
    }
  };
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const { triggerNavigation } = initBottomNav(pageType) || { triggerNavigation: null };

  const cta = document.getElementById('ctaButton');
  if (cta) {
    cta.addEventListener('click', () => {
      if (triggerNavigation) { triggerNavigation('/webapp/dashboard'); }
      else { window.location.href = '/webapp/dashboard'; }
    });
  }
