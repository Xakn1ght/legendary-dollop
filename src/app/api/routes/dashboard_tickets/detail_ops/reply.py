...
async def handle_dashboard_tickets_reply(request: web.Request):
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            # The assistant may answer straight away; every gate (switch,
            # human present, escalation words, rate limit) lives in the
            # service, and it stays silent unless all of them pass.
            try:
                from app.services.support_assist import maybe_answer_ticket
                await maybe_answer_ticket(session, ticket, user, message)
            except Exception:
                pass
